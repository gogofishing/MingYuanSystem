	...
	public SetJSR() {// 设置经手人面板
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(final MouseEvent e) {
				...
				nameStr = table.getValueAt(selRow, 1).toString();
				ageStr = table.getValueAt(selRow, 3).toString();
				sexStr = table.getValueAt(selRow, 2).toString();
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int op = JOptionPane.showConfirmDialog(SetJSR.this, "确认删除该操作员?");
				if(op == JOptionPane.OK_OPTION) {
					int selRow = table.getSelectedRow();
					if(selRow < 0)
						return;
					String id = table.getValueAt(selRow, 0).toString().trim();
					int del = Dao.delete("delete from tb_jsr where id ='" + id + "'");
					if(del > 0)
						JOptionPane.showMessageDialog(SetJSR.this, "删除成功");// 弹出提示框
